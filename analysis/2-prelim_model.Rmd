...
```{r}
# Set training task
task<- TaskClassif$new (id = "neckpain", backend = comb, target = "imp_np")
task$nrow
task$feature_names
...
ps_xgb <- ParamSet$new(list (
  ParamDbl$new ("xgb.nrounds", lower = 1, upper = 1000),
  ParamDbl$new ("xgb.lambda", lower = 0, upper = 1000),
  ParamDbl$new ("xgb.gamma", lower = 0, upper = 1000),
  ParamDbl$new ("xgb.alpha", lower = 0, upper = 1000)
))

ps_xgb$trafo = function(x, param_set) {
  x$xgb.nrounds = as.integer (x$xgb.nrounds)
  x$xgb.lambda = as.integer (x$xgb.lambda)
  x$xgb.alpha = as.integer (x$xgb.alpha)
  x
}
