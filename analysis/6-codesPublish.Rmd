...
perf_step2 <- readRDS("output/stepwise_performance2.RDS")
```
...
base_res <- perf_step2$ap$class_test# np, ap, dis


p_list <- vector ("list", 6)

for (n in seq_along (c(2:7))) {
  
  m <- n + o
  
  pred1 <- as.data.table (bmr2$prediction[[m]]) %>%
    as.data.frame() %>%
    dplyr::select (truth, response) %>%
    mutate (acc = ifelse (truth == response, 1, 0)) %>%
    pull (acc)
  
  xtabs <- table (base_res, pred1)
  
  p_list[[n]] <- mcnemar.test(xtabs)
